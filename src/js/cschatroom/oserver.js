var oServer = (function (window, r) {
    ...
    function switchSt (status) {
        $('.switch ul').hide();
        $('#csloading').show();
        if (oServer.status == status) return;
        ...
        ChatApi.doSignInOrOut({
            ...
            if (oServer.status == 1 || oServer.status == 2) {
                oServer.pollMsg();
            }
            if (oServer.status == 0) {
                oServer.alwaySt();
    ...
    function getLineNum (){
        _self = this;
        ChatApi.doWaitQueueLength().done(function (res) {
            if(csPublic.isError(res)) return;
